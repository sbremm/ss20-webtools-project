...
const ComponentN = ({
  ...
  useEffect(() => {
    ...
    const adjustedData = PCA.computeAdjustedData(
      data,
      principalComponents[Number(n) - 1],
    ).adjustedData[0]

    // global minimum and maximum for all components
    const min = Math.min(
      ...PCA.computeAdjustedData(data, principalComponents[0]).adjustedData[0],
      ...PCA.computeAdjustedData(data, principalComponents[1]).adjustedData[0],
    )
    const max = Math.max(
      ...PCA.computeAdjustedData(data, principalComponents[0]).adjustedData[0],
      ...PCA.computeAdjustedData(data, principalComponents[1]).adjustedData[0],
  ...
  const percentageExplained =
    data.length >= 2 && principalComponents.length >= 2
      ? PCA.computePercentageExplained(
          principalComponents,
          principalComponents[Number(n) - 1],
        )
      : 0
    ...
    >
      <Card bg="light">
        ...
        <Card.Body>
          ...
          Explains {Number(percentageExplained * 100).toFixed(0)}% of the
          variance.
          <HelpButton
